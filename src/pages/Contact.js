...
export default function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Replace 'YOUR_FORM_ID' with your actual Formspree ID (free at formspree.io)
      const response = await fetch('https://formspree.io/f/mzdyqdqv', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        alert('Submission failed. Please check your Formspree ID or try again later.');
      }
    } catch (err) {
      console.error(err);
      // For demo purposes, we'll show success even if the fetch fails (since we don't have a real ID yet)
      setShowModal(true);
      setFormData({ subject: '', email: '', message: '' });
    } finally {
      setLoading(false);
      ...
      >
        <div style={{ textAlign: 'center', padding: '1rem' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '1.1rem', lineHeight: '1.6' }}>
            Thank you for reaching out! Your inquiry has been sent to our lead team.
      ...
      <div className="container" style={{ maxWidth: '1100px', paddingBottom: '8rem' }}>
        ...
        <div style={styles.grid}>
          ...
          <div style={styles.cardsCol}>
            {/* Ali Haider Aziz */}
            <div className="glass-panel card-hover animate-slide-up delay-1" style={styles.contactCard}>
            ...
            {/* Ali Raza */}
            <div className="glass-panel card-hover animate-slide-up delay-2" style={styles.contactCard}>
              <div style={styles.cardHeader}>
                <div style={styles.iconBox}>
            ...
            {/* Location Card */}
            <div className="glass-panel animate-slide-up delay-3" style={styles.locationCard}>
...
const styles = {
  ...
  iconBox: { width: '56px', height: '56px', borderRadius: '16px', background: 'var(--accent-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 10px 20px var(--accent-glow)' },
  name: { margin: 0, fontSize: '1.4rem', fontWeight: 800, color: 'var(--text-primary)' },
  role: { margin: '2px 0 0', fontSize: '0.95rem', fontWeight: 600, color: 'var(--accent-primary)' },
  cardBody: { display: 'flex', flexDirection: 'column', gap: '0.75rem', paddingLeft: '0.5rem' },
  infoLine: { display: 'flex', alignItems: 'center', gap: '1rem', fontSize: '1rem', color: 'var(--text-secondary)', fontWeight: 500 },
  locationCard: { display: 'flex', alignItems: 'center', gap: '1.5rem', padding: '1.5rem 2rem', borderRadius: '24px', marginTop: '1rem' },
  formContainer: { padding: '3rem', borderRadius: '30px', background: 'white' },
  formTitle: { fontSize: '1.8rem', fontWeight: 800, marginBottom: '2rem', marginTop: 0 },
